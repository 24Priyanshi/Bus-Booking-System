...
def book_bus():
    ...
    Label(frame3, text="Fill Passenger details to book the Bus ticket", fg="red", relief="ridge", bg="indianred2",
          font=('poppins 20 bold')).grid(row=0, column=1, padx=10, pady=20, columnspan=10)

    #name of passenger
    Label(frame3, text="Full Name : ", font=('Helvetica 15 bold')).grid(row=1, column=0)
    global name_entry
    name_entry = Entry(frame3)
    name_entry.grid(row=1, column=1)

    #drop down menu of gender
    Label(frame3, text="Gender : ", font=('Helvetica 15 bold')).grid(row=1, column=2)
    ...
    Label(frame3, text="Total Seats: ", font=('Helvetica 15 bold')).grid(row=1, column=4)
    global seat_entry
    ...
    Label(frame3, text="Mobile no. : ", font=('Helvetica 15 bold')).grid(row=1, column=6)
    global mobile_entry
    mobile_entry = Entry(frame3)
    mobile_entry.grid(row=1, column=7)

    #age 
    Label(frame3, text="Age : ", font=('Helvetica 15 bold')).grid(row=1, column=8)
    global age_entry
    age_entry = Entry(frame3)
    age_entry.grid(row=1, column=9, padx=10)


    Button(frame3, text="Book Seat", fg="red", bg="cyan3", font=('Helvetica 15 bold'),command=passenger_data).grid(row=10, column=10)
...
def passenger_data():
    ...
    if (not ( total_seat and mobile_no and age)):
            messagebox.showwarning("Warning", " fill all entry first ")
            return
    ...
    if int(age)>100 :
        messagebox.showerror("error"," Age Should be less than 100")
        return

    if int(age)<10 :
        messagebox.showerror("error"," Age Should be aleast 10")
        return
    ...
    if not(total_seat.isdigit()) :
        ...
        return 
    if(int(total_seat)>5) :
        messagebox.showerror("error"," maximum seat 5 seat can be booked")
        return
    if(int(total_seat)<=0) :
        messagebox.showerror("error"," min seat 1 seat should be booked")
        return   
...
def show_bus():
    ...
    Label(frame2,text="Select Bus",fg="lightyellow2",font=('Helvetica 15 bold')).grid(row=4,column=1,padx=10)
    
    Label(frame2,text="Operator",fg="chocolate1",font=('Helvetica 15 bold')).grid(row=4,column=2,padx=10)
    
    Label(frame2,text="Bus Type",fg="hotpink2",font=('Helvetica 1 5bold')).grid(row=4,column=3,padx=10)

    Label(frame2,text="Available/Capacity",fg="yellow3",font=('Helvetica 15 bold')).grid(row=4,column=4,padx=10)

    Label(frame2,text="Fare",fg="purple2",font=('Helvetica 15 bold')).grid(row=4,column=5,padx=10)
...
def display_buses(buses_running):
    ...
    for i in range(len(buses_running)):
        ...
        Radiobutton(frame2, text=f'Bus {i + 1}', bg="light blue", font=('Helvetica 15 bold'),
        ...
        l = l + 1
        Label(frame2, text=operator_name[0], fg="blue", font=('Helvetica 15 bold')).grid(row=k, column=l, padx=10)
        bus_type = bus_booking_system.show_bus_type(bus_id)
        l = l + 1
        Label(frame2, text=bus_type[0], fg="blue", font=('Helvetica 15 bold')).grid(row=k, column=l, padx=10)
        available = bus_booking_system.show_avail(bus_id,Journey_date)
        cap = bus_booking_system.show_cap(bus_id)
        l = l + 1
        Label(frame2, text=f'{available}/{cap[0]}', fg="blue", font=('Helvetica 15 bold')).grid(row=k, column=l,
                                                                                                       padx=10)
        l = l + 1
        fare = bus_booking_system.show_fare(bus_id)
        Label(frame2, text=fare[0], fg="blue", font=('Helvetica 15 bold')).grid(row=k, column=l, padx=10)
        k = k + 1
        l = 1

    Button(frame2, text="Proceed to Book", bg="red", font=('Helvetica 15 bold'), command=book_bus).grid(row=k-1, column=7)
...
title_project= Label(frame2,text="Online Bus Booking System",relief="ridge",bg="light blue",fg="red",font=('Helvetica 40 bold'))
title_project.grid(row=0,column=0,pady=20,columnspan=20)


Label(frame2,text="Enter Journey Details",bg="green",font=('Helvetica 20')).grid(row=1,column=0,pady=20,columnspan=20)

Label(frame2,text="From:",font=('Helvetica 15 bold')).grid(row=3,column=0)

start=Entry(frame2)
start.grid(row=3,column=1)
Label(frame2,text="To:",font=('Helvetica 15 bold')).grid(row=3,column=2,padx=10)
finish=Entry(frame2)
finish.grid(row=3,column=3)

Label(frame2,text='''Journey Date:(DD/MM/YYYY)''',font=('Helvetica 15 bold')).grid(row=3,column=4,padx=10)
date=Entry(frame2)
date.grid(row=3,column=5)
#show bus button
Button(frame2,text="Show Bus",bg="royalblue1",font=('Helvetica 15 bold'),command=show_bus).grid(row=3,column=6,padx=10)
# home button



home_button=Button(frame2,text="HOME",font=('Helvetica 15 bold'),bg="antique white",command=homepage)
